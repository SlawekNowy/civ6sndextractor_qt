...
class Packet
{
    ...
    bool _no_granule;
public:
    Packet(ifstream& i, long o, bool little_endian, bool no_granule = false) : _offset(o), _size(0xFFFF), _absolute_granule(0), _no_granule(no_granule) {
        i.seekg(_offset);
...
class Packet_8
{
    ...
    uint32_t _absolute_granule;
public:
    Packet_8(ifstream& i, long o, bool little_endian) : _offset(o), _size(0xFFFFFFFF), _absolute_granule(0) {
        i.seekg(_offset);
    ...
    _read_32(nullptr)
{
    ...
    _file_size = static_cast<long>(_infile.tellg());
    ...
    while (chunk_offset < _riff_size)
    {
	    ...
	    uint32_t chunk_size = _read_32(_infile);
    ...
    switch(_vorb_size)
    {
        ...
        case 0x34:
            ...
            _blocksize_0_pow = static_cast<uint8_t>(_infile.get());
            _blocksize_1_pow = static_cast<uint8_t>(_infile.get());
            break;
    }
...
void Wwise_RIFF_Vorbis::generate_ogg_header(Bit_oggstream& os, bool * & mode_blockflag, int & mode_bits)
{
    ...
    // generate comment packet
    {
        ...
        else
        {
	        ...
	        Bit_uint<32> loop_start_comment_length = static_cast<Bit_uint<32>>(loop_start_str.str().length());
	        ...
	        Bit_uint<32> loop_end_comment_length = static_cast<Bit_uint<32>>(loop_end_str.str().length());
    ...
    // Audio pages
    {
        ...
        while (offset < _data_offset + _data_size)
        {
            ...
            if (_mod_packets)
            {
                ...
                Bit_uintv * mode_number_p;
                Bit_uintv * remainder_p;
